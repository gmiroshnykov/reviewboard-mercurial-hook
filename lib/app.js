...
/**
 * This is the main function that handles incoming changesets during push.
 ...
 */
function processIncomingChangesets(bugId, node, callback) {
  ...
  return async.auto({
    ...
    newReviewRequests: ['newChangesets', function(cb, results) {
      getOrCreateReviewRequests(results.newChangesets, cb);
    }]
...
exports.processIncomingChangesets = processIncomingChangesets;
...
function getOrCreateReviewRequests(changesets, callback) {
  return async.mapLimit(changesets,
    config.ASYNC_LIMIT,
    getOrCreateReviewRequest,
    callback);
}
...
function createReviewRequest(changeset, callback) {
  ...
  return reviewboard.createReviewRequest(options, function(err, reviewRequest) {
    ...
    return createReviewRequestDraft(reviewRequest.id, changeset, callback);
  });
}
