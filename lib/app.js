...
function processChangegroup(username, password, bugId, node, callback) {
  ...
  return async.auto({
    ...
    newReviewRequests: ['parentReviewRequest', 'baseChangeset', 'newChangesets', function(cb, results) {
      updateReviewRequests(results.baseChangeset, bugId, results.newChangesets, cb);
    }]
...
function updateReviewRequests(baseChangeset, bugId, changesets, callback) {
  return async.mapLimit(changesets,
    config.ASYNC_LIMIT,
    updateReviewRequest.bind(null, baseChangeset, bugId),
    callback);
}

function updateReviewRequest(baseChangeset, bugId, changeset, callback) {
  return getOrCreateReviewRequest(changeset, function(err, reviewRequest) {
    ...
    return createReviewRequestDiff(reviewRequestId,
      ...
      function(err) {
        ...
        return createReviewRequestDraft(reviewRequestId, bugId, changeset,
          function(err, reviewRequestDraft) {
            if (err) return callback(err);

            // read back fresh review request details
            return getReviewRequest(reviewRequestId, callback);
          }
        );
      }
    );
...
function getOrCreateReviewRequest(changeset, callback) {
  return getReviewRequestByChangeset(changeset, function(err, reviewRequest) {
    if (err) return callback(err);
...
function getReviewRequest(id, callback) {
  return reviewboard.getReviewRequest(id, function(err, reviewRequest) {
    if (err) return callback(err);
    if (!reviewRequest) return callback(null, null);
    return populateReviewRequest(reviewRequest, callback);
  });
}

function getReviewRequestByChangeset(changeset, callback) {
  var options = {
    commitId: changeset.node
  };
  return reviewboard.findReviewRequest(options, function(err, reviewRequest) {
    if (err) return callback(err);
    if (!reviewRequest) return callback(null, null);
    return populateReviewRequest(reviewRequest, callback);
  });
  ...
  changeset, callback)
{
  ...
  return getParentDiff(baseChangeset, changeset, function(err, parentDiff) {
    ...
    return repository.diff(options, function(err, diff) {
      ...
      if (parentDiff !== false) {
        options['parentDiffPath'] = parentDiff;
      }

      return reviewboard.uploadDiff(reviewRequestId, options, callback);
    });
  });
}
...
function getParentDiff(baseChangeset, changeset, callback) {
  return getParentChangeset(changeset.node, function(err, parentChangeset) {
    if (err) return callback(err);
    if (baseChangeset.node === parentChangeset.node) {
      // base changeset is our parent changeset, so we don't need parent diff
...
function createReviewRequestDraft(reviewRequestId, bugId, changeset, callback) {
  var options = {
    commitId: changeset.node,
  ...
  return reviewboard.createReviewRequestDraft(reviewRequestId, options, function(err) {
    if (err) return callback(err);
...
function getOrCreateParentReviewRequest(bugId, callback) {
  return getParentReviewRequestByBugId(bugId, function(err, parentReviewRequest) {
    if (err) return callback(err);
...
function populateReviewRequest(reviewRequest, callback) {
  return reviewboard.getReviewRequestDraft(reviewRequest.id, function(err, reviewRequestDraft) {
    if (err) return callback(err);
    if (!reviewRequestDraft) return callback(null, reviewRequest);

    delete reviewRequestDraft.id;
    delete reviewRequestDraft.links;
    for (var k in reviewRequestDraft) {
      reviewRequest[k] = reviewRequestDraft[k];
    }
...
function getParentReviewRequestByBugId(bugId, callback) {
  // FIXME: RB API won't allow us to search for existing review requests
  // by bug number, so we have to abuse commit_id field instead
  var options = {
    commitId: 'bug-' + bugId
  ...
  return reviewboard.findReviewRequest(options, function(err, parentReviewRequest) {
    if (err) return callback(err);
    if (!parentReviewRequest) return callback(null, null);
    return populateReviewRequest(parentReviewRequest, callback);
  });
}
